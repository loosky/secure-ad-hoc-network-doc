...
The design here is based on a working pro-active ad hoc routing protocol. The
...
application, but lets assume that the actors carrying their communication
devices, hereafter nodes, physically meets the \ac{SP} at the scene and exchange
their public key fingerprints.
...
the fingerprint request a proxy certificate itself. The \ac{SP} will after
verifying the nodes fingerprint be able to issue a proxy certificate with at
least the rights to participate in building the \ac{MANET} by broadcasting its
own and re-broadcasting other trusted nodes routing announcements.
...
easier to acheive when the \ac{SP} can delegete its own rights.
...
they are used in, but here is a few possible rights/priveleges to give the
...
personell, you could have some actors be able to organize the effort by sending
...
\subsection{How routing occurs hereafter, neighbor checking\ldots}
...
BATMAN will be expained in Chapter \ref{chapter_implementation}.
...
its \ac{AL}, or one can rather say it has an empty \ac{AL} - denoted by the 'Ø'
in the figure.
