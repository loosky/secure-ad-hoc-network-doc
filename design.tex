...
and corresponding IV, a nonce, and a signature (HMAC) of this message. It is
important to note that the ephemeral key is encrypted with the neighbor's public
key (hence multiple unicasts instead of an actual broadcast), but the HMAC is
generated using the unencrypted key (to avoid generating a HMAC for each
neighbor).
...
TODO: Skriv om  at jeg legger ny nabo i nabolista også blabla\ldots
...
its \ac{AL}, or one can rather say it has an empty \ac{AL} - denoted by the 'ï¿½'
in the figure.
